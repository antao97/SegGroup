...
class Dataset:
    ...
    def trainLoader(self):
        ...
        if cfg.type == '1/11':
            train_file_names = glob.glob(os.path.join(self.data_root, self.dataset, 'train', '*' + self.filename_suffix))
            random.shuffle(train_file_names)
            train_file_names = sorted(train_file_names[:round(len(train_file_names)*1.93/22.3)])
        else:
            train_file_names = sorted(glob.glob(os.path.join(self.data_root, self.dataset, 'train_'+cfg.type, '*' + self.filename_suffix)))
