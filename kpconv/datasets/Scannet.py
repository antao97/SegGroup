...
class ScannetDataset(Dataset):
    ...
    def __init__(self, input_threads=8, load_test=False):
        ...
        # self.train_clouds = np.loadtxt(join(self.path, 'scannet_v2_train.txt'), dtype=np.str)
        # print(len(self.train_clouds))
        # random.seed(0)
        # random.shuffle(self.train_clouds)
        # self.train_clouds = np.sort(self.train_clouds[:round(len(self.train_clouds)*1.93/22.3)])
        # print(len(self.train_clouds))
        # self.validation_clouds = np.loadtxt(join(self.path, 'scannet_v2_val.txt'), dtype=np.str)
        # self.train_files = [join(self.train_path, f + '.ply') for f in np.concatenate([self.train_clouds, self.validation_clouds])]
